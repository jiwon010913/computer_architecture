...
int find_lru_block(CacheBlock* cache, int index, int assoc);
...
void solution(int cache_size, int block_size, int assoc) {
    ...
    instruction_cache = calloc(blocks, sizeof(CacheBlock));
    data_cache = calloc(blocks, sizeof(CacheBlock));
...
void read_op(int addr, int cache_size, int block_size, int assoc){
    int total_set = cache_size / (block_size * assoc);
    int index = (addr / block_size) % total_set;
    int tag = (addr/block_size)/total_set;

    int i;
    for (i = 0; i < assoc; i++) {
        if (data_cache[index*assoc + i].valid && data_cache[index*assoc + i].tag == tag) {
            // Cache hit
            data_cache[index*assoc + i].timestamp = time_count;
            time_count++;
            return;
        }
    }
    ...
    if (data_cache[index*assoc + lru_block].dirty) //이걸 왜 확인해? write back의 경우... dirty bit 확인해
        d_write++;  // Write back to memory if the block is dirty

    // Update cache with new block    
    data_cache[index*assoc + lru_block].valid = 1;
    data_cache[index*assoc + lru_block].tag = tag;
    data_cache[index*assoc + lru_block].dirty = 0;
    data_cache[index*assoc + lru_block].timestamp = time_count;
    time_count++;
	return;
}
...
void write_op(int addr, int cache_size, int block_size, int assoc){
    ...
    for (i = 0; i < assoc; i++) {
        if (data_cache[index*assoc + i].valid && data_cache[index*assoc + i].tag == tag) {
            // Cache hit
            data_cache[index*assoc + i].timestamp = time_count;
            data_cache[index*assoc + i].dirty  = 1;
            time_count++;
            return;
        }
    }
    ...
    int lru_block = find_lru_block(data_cache, index, assoc);
    if (data_cache[index*assoc + lru_block].dirty) 
        d_write++;  // Write back to memory if the block is dirty

    // Update cache with new block
    data_cache[index*assoc + lru_block].valid = 1;
    data_cache[index*assoc + lru_block].tag = tag;
    data_cache[index*assoc + lru_block].dirty = 1;
    data_cache[index*assoc + lru_block].timestamp = time_count;
    time_count++;
	return;
}
...
void fetch_inst(int addr, int cache_size, int block_size, int assoc){
    ...
    int i;
    for (i = 0; i < assoc; i++) {
        if (instruction_cache[index*assoc + i].valid && instruction_cache[index*assoc + i].tag == tag) {
            // Cache hit
            instruction_cache[index*assoc + i].timestamp = time_count;
            time_count++;
            return;
        }
    }
    ...
    // Update cache with new block
    instruction_cache[index*assoc + lru_block].valid = 1;
    instruction_cache[index*assoc + lru_block].tag = tag;
    instruction_cache[index*assoc + lru_block].dirty = 0;
    instruction_cache[index*assoc + lru_block].timestamp = time_count;
    time_count++;
	return;
}
...
int find_lru_block(CacheBlock* cache, int index, int assoc) {
    int lru_block = 0;
    int min_timestamp = time_count+1;
    int i;
    for (i = 0; i < assoc; i++) {
        if (cache[index*assoc + i].timestamp < min_timestamp) {
            lru_block = i;
            min_timestamp = cache[index*assoc + i].timestamp;
        }
    }
    return lru_block;
}
